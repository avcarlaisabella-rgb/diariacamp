      ...
      <div className="hidden sm:block bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        <table className="w-full text-left text-xs table-fixed">
          <thead className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200">
            <tr>
              <th className="w-[24%] px-3.5 py-3">Usuário & E-mail</th>
              <th className="w-[13%] px-3 py-3">Região / Zona</th>
              <th className="w-[13%] px-3 py-3">Hierarquia (Gestor)</th>
              <th className="w-[10%] px-2 py-3 text-center">Perfil</th>
              <th className="w-[8%] px-2 py-3 text-center">Status</th>
              {isAdmin && <th className="w-[32%] px-3 py-3 text-right">Ações</th>}
            </tr>
          </thead>
              ...
              return (
                <tr key={u.id} className={`hover:bg-slate-50/80 transition-colors ${!isActive ? 'bg-slate-50/50' : ''}`}>
                  ...
                  <td className="px-1.5 py-3 text-center overflow-hidden">
                    <span className={`px-1.5 py-1 rounded-full text-[9px] font-bold inline-block max-w-full truncate align-middle ${
                      u.role === 'admin' ? 'bg-purple-100 text-purple-800' :
                      u.role === 'gestor' ? 'bg-blue-100 text-blue-800' : 'bg-emerald-100 text-emerald-800'
                    }`}>
                      {getRoleLabel(u.role)}
                  ...
                  <td className="px-1.5 py-3 text-center overflow-hidden">
                    <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold inline-block max-w-full truncate align-middle ${
                  ...
                  {isAdmin && (
                    <td className="px-2 py-3.5 text-right whitespace-nowrap overflow-hidden">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          className="px-1.5 py-1 bg-blue-50 hover:bg-blue-100 active:scale-95 text-blue-700 border border-blue-200/80 rounded-lg text-[10px] font-semibold transition-colors cursor-pointer inline-flex items-center gap-1 shrink-0"
                          title="Alterar Senha deste Usuário (Exclusivo Administrador)"
                        >
                          <KeyRound className="w-3.5 h-3.5 text-blue-600" />
                          <span>Senha</span>
                        ...
                        <button
                          onClick={() => toggleUserActive(u.id)}
                          className={`px-1.5 py-1 rounded-lg text-[10px] font-bold transition-colors cursor-pointer shrink-0 ${
                            isActive
